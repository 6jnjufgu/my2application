package com.bwie.information.myapplication.adapter;
...
import com.bwie.information.myapplication.base.BaseViewHolder;
import com.bwie.information.myapplication.bean.Data;
import com.bwie.information.myapplication.util.TypeUtil;
...
public class MAdapter extends RecyclerView.Adapter<BaseViewHolder> implements TypeUtil {
    ...
    private void initOtherData() {
        for (int i = 0; i < databean.getData().getForum_list().size(); i++) {

            databean.getData().getForum_list().get(i).getGroup().get(0).setHasTitle(true,databean.getData().getForum_list().get(i).getName());//设置有标头
            for (int j = 0; j < databean.getData().getForum_list().get(i).getGroup().size(); j++) {
                j++;
                Pair<Object, Object> objectObjectPair;//两个数据的集合  超过两个可以用list集合
                if (j == databean.getData().getForum_list().get(i).getGroup().size()) {
                    objectObjectPair = wrapData(databean.getData().getForum_list().get(i).getGroup().get(j - 1), null);
                } else {
                    objectObjectPair = wrapData(databean.getData().getForum_list().get(i).getGroup().get(j - 1), databean.getData().getForum_list().get(i).getGroup().get(j));
                }
